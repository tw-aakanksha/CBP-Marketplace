...
const ValidationReact = () =>{
    ...
    const passwordHandler =(e) =>{
        let item=e.target.value;
        if(!item.match(/^(?=.*\d)(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z]).{8,}$/) || item.length<0)
        {
            setPasswordErr(true)
        }
        else
    ...
    <div className='signup-buyer-container'>
            ...
            <input
                ...
                autoComplete= 'off'
             />
             <div className='text-danger'> {emailErr?<span>Email Id is not Valid, Please enter valid email</span>:""} </div>
          ...
          <br></br>
            ...
            <div className='text-danger'> {displayNameErr?<span>Display name should contain minimum 4 charcters and Maximum of 16 charcters</span>:""} </div>
           <br></br>
          ...
          <label>Password</label>
           ...
           <div className='text-danger'>  {passwordErr?<span>Password should contain atleast one uppercase,one lowercase,one letter and a special character with minimum of length 8 characters</span>:""}</div>
           ...
           <br></br>
            ...
            <div className='text-danger'> {confirmPasswordErr?<span>Password and Confirm Password are not matching</span>:""}</div>
